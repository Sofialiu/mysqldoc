...
>[AZURE.NOTE]**需要注意的是连接池中或长链接也存在时效性，服务器端会设置超时机制，如果一个连接在一段时间内处于闲置状态（没有任何请求），服务器就会关闭这个链接，以释放不必要的资源占用。因此，为了更好的维护连接池，保障MySQL数据库的访问速度，用户需要在客户端配置验证机制，用以进行连接有效性的验证，保证分配的连接都是有效的，若失效，客户端需重新建立一个新的连接并返回。以Java为例，用户可以参考[JDBC Connection Pool官方介绍文档](https://tomcat.apache.org/tomcat-7.0-doc/jdbc-pool.html#Common_Attributes)中ValidationQuery的设定。**
